## CSAM：用于各向异性体积医学图像分割的 2.5D 交叉切片注意力模块
## https://github.com/aL3x-O-o-Hung/CSAM/tree/main
from torchvision import transforms
# from VM_UNet.utils import *
from utils import *
...
class setting_config:
    ...
    network = 'xlstm_vmunet'         # todo:'vmunet';'vmunet_Sobel'; 'vmunet_CSAM_Sobel'; 'hmtunet_Sobel';'xlstm_vmunet'
    if network == 'mambaunet':
        model_config = {
            ...
            # -----  MambaUNet ----- #
            'load_ckpt_path': '/data/glw/Projects/Mamba_UNet/code/pretrained_ckpt/vmamba_tiny_e292.pth',
            'patch_size':4,
            'embed_dim':96,
            'vssm_depths': [2, 2, 9, 2],
            'swin_mlp_ratio':4.,
        ...
        model_config = {
            ...
            'load_ckpt_path':None,
            # # todo:【注释掉】如果不加载预训练模型
            # 'load_ckpt_path': '/data/glw/Projects/VM_UNet/pre_trained_weights/vssm_small_0229_ckpt_epoch_222.pth',     # vssmsmall_upernet_4xb4-160k_ade20k-640x640_iter_112000.pth

            # train in the second time
            # 'load_ckpt_path': '/home/data/glw/Projects/VM_UNet/pre_trained_weights/polyp_latest_STNHausdorff.pth',
        }

    datasets = 'cartilage_pig'   #todo: 'cartilage' # 'polyp'  ,'kvasir'         # set dataset path
    if datasets == 'isic18':
        data_path = './data/isic2018/'
    elif datasets == 'isic17':
        data_path = './data/isic2017/'
    elif datasets == 'cartilage':
        data_path = '/data/glw/Projects/mmsegmentation/projects/KneeCartilage02_UNet/CartilageData_202409_202411/'
        # data_path = '/data/glw/Projects/CartilageData_endoscope/'    # todo:[此时要修改VM_UNet/datasets/dataset.py 中第42行的mask转化为0，1数值的式子（mask/256）]
        # data_path = '/data/glw/Projects/VM_UNet/data_medaugment/'
    elif datasets == 'polyp':
        # data_path = '/data/glw/Projects/PolypDatasets/'
        data_path = '/data/glw/Projects/VM_UNet/polyp_medaugment/' # todo: medaugment 增强处理后的数据集
    elif datasets == 'clinicDB':
        data_path = '/data/glw/Projects/PolypDatasets/ClinicDB_Train/'
    elif datasets == 'kvasir':
        data_path = '/data/glw/Projects/PolypDatasets/Kvasir_Train/'
    elif datasets == 'cartilage_pig':
        data_path = '/home/csd/datasource/cartilage_pig/'
    else:
    ...
    # criterion = HausdorffDiceLoss(alpha=0.3, wb=1, wd=1)
    ...
    print_interval = 20
    val_interval = 20        # 30
    save_interval = 1       # output interval during testing
    threshold = 0.5
    ...
    if opt == 'Adadelta':
        ...
        lr = 0.001 # default: 1e-3 – learning rate
        betas = (0.9, 0.999) # default: (0.9, 0.999) – coefficients used for computing running averages of gradient and its square
        eps = 1e-6 # default: 1e-8 – term added to the denominator to improve numerical stability
