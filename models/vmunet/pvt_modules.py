...
from models.vmunet.pvtv2_modules import pvt_v2_b2
...
class BasicConv2d(nn.Module):   # upsample layer
...
class CFM(nn.Module):
    def __init__(self, channel):
        ...
        # self.fc1 = nn.Conv2d(in_channels=channel, out_channels=channel // 2, kernel_size=1)
        # 在 CFM 模块中进行了上采样
        self.upsample = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
        self.conv_upsample1 = BasicConv2d(channel, channel//2, 3, padding=1)
        self.conv_upsample2 = BasicConv2d(channel, channel//4, 3, padding=1)
        self.conv_upsample3 = BasicConv2d(channel//2, channel//4, 3, padding=1)
        self.conv_upsample4 = BasicConv2d(channel, channel//2, 3, padding=1)
        self.conv_upsample5 = BasicConv2d(channel, channel//4, 3, padding=1)

        self.conv_concat2 = BasicConv2d(channel, channel, 3, padding=1)
        self.conv_concat3 = BasicConv2d(channel//2, channel//4, 3, padding=1)
        self.conv4 = BasicConv2d(channel//4, channel//8, 3, padding=1)
    ...
    def forward(self, x1, x2, x3):
        # 通道数：x1-768,x2-384，x3-192，根据输入输出的维度调整conv_upsample中维度的除数
        x1_1 = x1
        ...
        x1 = self.conv4(x3_2)
        # 使用双线性插值进行上采样
        x1 = F.interpolate(x1, size=(x1.shape[-2]*2, x1.shape[-1]*2), mode='bilinear', align_corners=True)
...
class SAM(nn.Module):
    def __init__(self, num_in=96, plane_mid=16, mids=4, normalize=False):  # todo: num_in=32 --> 96
...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        out = avg_out + max_out  # (1,96,1,1)
        return self.sigmoid(out)  # (1,96,1,1)
...
class PolypPVT(nn.Module):  # 没有被调用
    def __init__(self, channel=32):
        super(PolypPVT, self).__init__()

        self.backbone = pvt_v2_b2()  #todo: 原[64, 128, 320, 512]  -->  [96, 192, 384, 768]
